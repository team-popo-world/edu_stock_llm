...
def get_game_scenario_prompt3():
    ...
    [
        ...
        "stocks": [
          {
            ...
            "initial_value": 100,
            "current_value": 100,
            "expectation": "[1턴 뉴스 기반의 다음 턴 예상 - 예: 보름달 밤에는 달빛 가루를 더 많이 모을 수 있을 것 같아요!]",
            "risk_level": "저위험"
          },
          {
            "name": "달조각 목걸이 훔치기 작전",
            ...
            "initial_value": 100,
            "current_value": 100,
            "expectation": "[1턴 뉴스 기반의 다음 턴 예상 - 예: 밝은 달빛 아래서 달조각 목걸이가 더욱 탐나게 빛날 거예요!]",
            "risk_level": "중위험"
          },
          {
            "name": "달빛 방패 강탈 작전",
            ...
            "initial_value": 100,
            "current_value": 100,
            "expectation": "[1턴 뉴스 기반의 다음 턴 예상 - 예: 보름달에는 왕궁 경비가 더 삼엄해져서 방패를 노리기 어려울지도 몰라요.]",
            "risk_level": "고위험"
          }
        ]
      },
      {
        ...
        "news_hint": "[2턴 뉴스의 영향 힌트 - 예: 연회에는 귀족들이 목걸이를 많이 하고 올테고, 왕궁 경비는 허술해질 수 있어요! 달빛 가루는 조용히 모을 수 있을까요?]",
        ...
        "stocks": [
          {
            ...
            "initial_value": 100,
            "current_value": "[1턴 결과가 반영된 값, 예: 104]",
            "expectation": "[2턴 뉴스 기반 예상 - 예: 연회가 열리면 경비들이 바빠서 달빛 가루를 모으기 더 쉬울지도 몰라요!]",
            "risk_level": "저위험"
          },
          {
            "name": "달조각 목걸이 훔치기 작전",
            ...
            "expectation": "[2턴 뉴스 기반 예상 - 예: 연회에 참석한 귀족들의 목걸이를 노릴 절호의 기회일 수 있어요!]",
            "risk_level": "중위험"
          },
          {
            "name": "달빛 방패 강탈 작전",
            ...
            "expectation": "[2턴 뉴스 기반 예상 - 예: 왕궁 연회로 경비가 허술해진다면 방패를 노릴 기회지만, 여전히 위험해요!]",
            "risk_level": "고위험"
          }
        ]
      }
      // ... (이하 10턴까지 동일한 구조로 반복, 각 작전의 'description' 필드는 위에 명시된 내용과 동일하게 유지)
    ]
    ```
    ...
    6.  **일관성**: 작전의 이름, `description`, 위험도, `initial_value`는 게임 내내 일관되어야 합니다. 각 작전의 `description` 필드는 위에 명시된 "JSON description" 내용을 모든 턴에서 동일하게 사용해주세요.
    7.  **`expectation` 필드**: 현재 턴의 `news`와 `news_hint`를 바탕으로 각 작전의 가치 변동 예상을 "가치가 오를 것 같아요!", "조금 떨어질 수도 있어요.", "큰 변화는 없을 거예요." 등으로 아이들이 이해하기 쉽게 구체적으로 작성합니다. 예시 JSON의 `expectation` 항목을 참고하여 작성해주세요.
    ...
    -   **News → Result → Value 흐름 일치**: 각 턴의 `news`가 다음 턴의 `result`에 논리적으로 반영되고, 이 `result`에 따라 `current_value`가 올바르게 변동되었는지 확인합니다.
    -   **작전 정보 일관성**: 3개의 작전("달빛 가루 수집 작전", "달조각 목걸이 훔치기 작전", "달빛 방패 강탈 작전")이 정확한 이름, `description` (위에 명시된 JSON description 내용과 동일), `risk_level`("저위험", "중위험", "고위험"), `initial_value`(100)로 모든 턴에 일관되게 등장하는지 확인합니다.
